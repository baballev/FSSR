...
def maml_update(model, lr, grads=None):
    ...
    Description
    ---
    Performs a MAML update on model using grads and lr. The function re-routes the Python object,
    thus avoiding in-place operations.
    ...
    * **grads** (list, *optional*, default=None) - A list of gradients for each parameter
    ...
    if grads is not None:
        ...
        if not len(grads) == len(list(params)):
            msg = 'WARNING:maml_update(): Params and gradients have different length: %s vs %s' \
...
class MAML(BaseLearner):
    ...
    Description
    ---
    High-level implementation of *Model-Agnostic Meta-Learning*. This class wraps an arbitrary
    nn.Module and augments it with `clone()` and `adapt()` methods. For the first-order version of
    MAML (i.e. FOMAML), set the `first_order` flag to `True` upon initialization.
